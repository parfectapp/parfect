...
const COURSES = {
  campestre: { id: 'campestre', name: 'Club Campestre Morelia', sub: '9 hoyos · Par 36', holes: CAMP_HOLES, nines: [{ label: '1-9', start: 0 }] },
  tresmarias: { id: 'tresmarias', name: 'Tres Marías · El Reto', sub: '27 hoyos · 3 vueltas', approx: true, holes: buildHoles(TM_PARS, TM_YDS, TM_DOGS), nines: [{ label: '1-9', start: 0 }, { label: '10-18', start: 9 }, { label: '19-27', start: 18 }] },
  altozano: { id: 'altozano', name: 'Altozano Morelia', sub: '18 hoyos · Par 72', approx: true, holes: buildHoles(ALT_PARS, ALT_YDS, ALT_DOGS).map((h, i) => Object.assign(h, { g: ALT_GREENS[i] })), nines: [{ label: '1-9', start: 0 }, { label: '10-18', start: 9 }] },
};
...
function roundOptions(cid) {
  ...
  const nines = c.nines || [{ label: '1-9', start: 0 }];
  ...
  const opts18 = [];
  if (total >= 18) {
    for (let i = 0; i + 18 <= total; i += 9) opts18.push({ holes: 18, start: i, label: `${nineLabel(i)} + ${nineLabel(i + 9)}` });
  } else if (total === 9) {
    opts18.push({ holes: 18, start: 0, label: '18 (repite 1-9)' });   // campos de 9: se juega doble vuelta
  }
  return { opts9, opts18, total };
}
/* par total de una ronda (offset + nº de hoyos) — da la vuelta si el campo es de 9 */
function roundPar(cid, start, holes) {
  const hs = COURSES[cid].holes; const n = hs.length; let p = 0;
  for (let i = 0; i < holes; i++) { const h = hs[(start + i) % n]; if (h) p += h.par; }
  return p;
}
...
function holeSchematic(hole, landings) {
  ...
  pts.forEach((q, i) => {
    ...
    lands += `<ellipse cx="${q.p[0].toFixed(0)}" cy="${q.p[1].toFixed(0)}" rx="${rx.toFixed(0)}" ry="${ry.toFixed(0)}" fill="#2f9e54" stroke="#2f9e54" stroke-width="1.5" stroke-dasharray="4 4" opacity="0"><animate attributeName="opacity" values="0;0;0.26;0.14;0.22" keyTimes="0;${tb};${ta};${tm};1" dur="${animDur}s" repeatCount="indefinite"/></ellipse>
      <text x="${q.p[0].toFixed(0)}" y="${(q.p[1] - ry - 5).toFixed(0)}" fill="#2f9e54" font-family="Inter,system-ui,sans-serif" font-size="11.5" font-weight="900" text-anchor="middle" opacity="0">${q.dist}y<animate attributeName="opacity" values="0;0;1;1" keyTimes="0;${tb};${ta};1" dur="${animDur}s" repeatCount="indefinite"/></text>`;
  });
  return `<svg width="100%" viewBox="0 0 ${W} ${H}" role="img" aria-label="Esquema hoyo ${hole.n}">
    ...
    <path d="M${gx + 5},${gy - 32} L${gx + 18},${gy - 28} L${gx + 5},${gy - 24} Z" fill="#2f9e54"/>
    ${risks}${lands}
    <path d="${route}" fill="none" stroke="#2f9e54" stroke-width="2.5" stroke-dasharray="3 6"/>
...
function simSchematic(hole, shots, shadowShots) {
  ...
  const colOf = sh => sh.ok ? '#2f9e54' : (sh.lie === 'water' ? '#2f7fa6' : '#ff9f43');
  let zones = '', dots = '';
  shots.forEach((sh, i) => {
  ...
  return `<svg width="100%" viewBox="0 0 ${W} ${H}" role="img" aria-label="Tiros del hoyo ${hole.n}">
    ...
    <path d="M${cx},${greenY - 30} l13,4 -13,4z" fill="#2f9e54"/>
    ${shadowG}
    ${zones}
    <path d="${route}" fill="none" stroke="#2f9e54" stroke-width="2" stroke-dasharray="3 5"/>
    ${dots}${ball}
...
function vSimulator() {
  ...
  if (!agg) {
    return `<div class="card empty"><div class="e-ico">${golfIcon('ball')}</div><h3>Necesitas tus stats</h3>
  ...
  if (!sim) {
    ...
    return `<div class="card">
      ...
      <button class="btn primary" data-act="sim-start" style="margin-top:14px">${golfIcon('club')} Jugar vs Shadow (${goalHcp === 0 ? 'Scratch' : 'HCP ' + goalHcp})</button>
    </div>`;
  }
  ...
  return `<div class="sec-h"><h2>${golfIcon('ball')} Simulador de ronda</h2><span class="small muted">ronda completa</span></div>
    <div class="card">
...
function objetivosCard(u) {
  ...
  return `<div class="card">
      <span class="label">${golfIcon('green')} Tu objetivo de esta semana</span>
...
function vEntreno() {
  ...
  const rows = M.map((x, i) => {
    ...
    return `<div class="mission ${st}">
      <div class="m-ic">${x.done ? '✓' : golfIcon(x.ic)}</div>
  ...
  return `<div class="sec-h"><h2>Tu campaña</h2><span class="small muted">${doneCount}/${M.length} misiones</span></div>
    <div class="card">
      <span class="label">${golfIcon('flag')} Misiones de mejora</span>
  ...
  return `<div class="sec-h"><h2>Campos</h2><span class="small muted">camino ideal por hoyo</span></div>
    ...
    <div class="card">
      ...
      ${reachable ? `<div style="border-top:1px solid var(--line-soft);margin-top:10px;padding-top:12px">
        <button class="chip ${attack ? 'on' : ''}" data-act="toggle-attack">${golfIcon('green')} Atacar el green en 2 ${attack ? '✓' : ''}</button>
